...
class HomeController extends Controller {
    ...
    protected function _initialize(){
        ...
        //判断当前用户是否具有后台登陆权限
        if(is_login()){
            $is_review = M("Agents")->where(array('id' => is_login()))->getField("is_review");
        }else{
            $is_review = 0;
        }

        $this->assign("is_review", $is_review);
	...
	/*分页*/
	protected function to_page($m, $map, $pagesize){
		$m = M($m); // 实例化User对象
		$map = $map;
        ...
        $list = $m->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("create_time DESC")->select();
